document.addEventListener("DOMContentLoaded", () => {
  ...
  if (window.gsap && !reduced) {
    ...
    if (window.ScrollTrigger) {
      gsap.utils
        .toArray(
          ".service-card,.industry-card,.process-step,.blog-card,.info-card,.contact-item,.dash-card,.photo-panel,.form",
        )
        .forEach((el, i) => {
          gsap.fromTo(
            el,
      ...
      gsap.utils
        .toArray(".section-head,.feature-copy,.auth-intro")
        .forEach((el) => {
          gsap.fromTo(
            el,
            { opacity: 0, x: -24 },
    ...
    $$(".logo-mark").forEach((el) => {
      el.addEventListener("mouseenter", () =>
        gsap.to(el, { rotation: 10, scale: 1.08, duration: 0.25 }),
      );
      el.addEventListener("mouseleave", () =>
        gsap.to(el, { rotation: 0, scale: 1, duration: 0.25 }),
      );
    });
    ...
    $$(".stat-number").forEach((el) => {
      ...
      if (window.ScrollTrigger) {
        gsap.to(obj, {
          ...
          scrollTrigger: { trigger: el, start: "top 88%", once: true },
          onUpdate: () => {
            el.innerHTML = Math.round(obj.value) + suffix;
          },
        });
      }
    });
  ...
  contactForm?.addEventListener("submit", (e) => {
    ...
    if (contactMsg) {
      contactMsg.textContent = "Request received. Redirecting...";
    }
  ...
  login?.addEventListener("submit", (e) => {
    e.preventDefault();
    const role = $("#loginRole", login),
      email = $("#loginEmail", login),
      pass = $("#loginPassword", login),
      msg = $(".auth-message", login);
    let ok = true;
    if (!role?.value) {
      showError(role, "Select Admin or Client.");
      ok = false;
    } else showError(role, "");
  ...
  const signup = $("#signupForm"),
    pw = $("#signupPassword");
  const rule = (n, v) =>
    document.querySelector(`[data-rule="${n}"]`)?.classList.toggle("valid", v);
  pw?.addEventListener("input", () => {
    const v = pw.value;
    rule("length", v.length >= 8);
    rule("upper", /[A-Z]/.test(v));
    rule("number", /[0-9]/.test(v));
    rule("special", /[^A-Za-z0-9]/.test(v));
  });
  signup?.addEventListener("submit", (e) => {
    e.preventDefault();
    const name = $("#signupName", signup),
      email = $("#signupEmail", signup),
      confirm = $("#signupConfirm", signup),
      terms = $("#terms", signup),
      msg = $(".auth-message", signup);
    let ok = true;
    if (!name?.value || !/^[A-Za-z .'-]{2,}$/.test(name.value)) {
      showError(name, "Use a valid name.");
      ok = false;
    } else showError(name, "");
    if (!email?.checkValidity()) {
    ...
    if (
      !pw?.value ||
      !/^(?=.*[A-Z])(?=.*[0-9])(?=.*[^A-Za-z0-9]).{8,}$/.test(pw.value)
    ) {
      showError(
        pw,
    ...
    if (msg)
      msg.textContent =
        "Account validated. Continue to Login to choose your role.";
  ...
  if (shell) {
    ...
    if (!role)
      try {
        role = (
          sessionStorage.getItem("cyvantaRole") ||
          localStorage.getItem("cyvantaRole") ||
          ""
        ).toLowerCase();
      } catch (e) {}
    if (!email)
      try {
        email =
          sessionStorage.getItem("cyvantaEmail") ||
          localStorage.getItem("cyvantaEmail") ||
          "";
      } catch (e) {}

    const expected = document.body.classList.contains("dashboard-admin")
      ? "admin"
      : "client";
    ...
    const sectionEmail = $("#sectionEmail"),
      sectionAvatar = $("#sectionAvatar");
    if (sectionEmail) sectionEmail.textContent = email;
    if (sectionAvatar)
      sectionAvatar.textContent = email.charAt(0).toUpperCase();
